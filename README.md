...
| Command | Description |
|---------|-------------|
| `/expense farm_id:<id> item:<item> quantity:<number> total:<amount> or price_per_item:<amount>` | Log a supply purchase for one farm. Farm IDs come from the farm dropdown. Monetary fields support `k`, `m`, and `b` suffixes like `1k` or `2.5m` |
| `/sale farm_id:<id> quantity:<number> total:<amount> or price_per_item:<amount>` | Log a Dried Kelp Block sale for one farm. Monetary fields support `k`, `m`, and `b` suffixes like `1k` or `2.5m` |
| `/balance farm_id:<id>` | View the current cycle's expenses, revenue, and per-player breakdown for one farm |
| `/payout farm_id:<id>` | Settle the cycle — shows each player's share and who needs to pay whom for one farm |
| `/history farm_id:<id>` | View transaction history, past cycle summaries, and a profit trend chart for one farm |
...
Create at least one farm with `/new farm farm_id:<id>` before logging expenses or sales.
